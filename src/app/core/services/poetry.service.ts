...
export class PoetryService {
    ...
    getAuthorData() {
        return this.authorValues.length ? of(this.authorValues) : this.http.get<any>('https://poetrydb.org/author').pipe(tap(data => this.authorValues = data.authors),
        map(data => {
            return data.authors;
       }))
    }
